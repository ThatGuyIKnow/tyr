...
def make_add_score_peak_memory_usage_bytes(max_memory_bytes: int):
    def add_scores(content, props):
        ...
        if "peak_memory_usage_bytes" not in props:
            props["score_peak_memory_usage_bytes"] = 0
            return
...
class SearchParser(Parser):
    ...
    def __init__(self, max_memory_bytes: int):
        ...
        self.add_pattern(
            "initial_h_value", r"\[.*\] Start node h_value: (\d+)", type=int
        )
        self.add_pattern(
            "initial_f_value", r"\[.*\] Start node f_value: (\d+)", type=int
        )
        ...
        self.add_pattern(
            ...
            r"\[Search\] Number of expanded states at last snapshot: (\d+)",
            type=int,
        )
        self.add_pattern(
            "num_generated_until_last_snapshot",
            r"\[Search\] Number of generated states at last snapshot: (\d+)",
            type=int,
        )
