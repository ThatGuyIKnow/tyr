...
if REMOTE:
    ENV = TetralithEnvironment(
        ...
        cpus_per_task=16,  # 16*2840 >= 32_000
...
for prefix, SUITE in SUITES:
    for task in suites.build_suite(BENCHMARKS_DIR / prefix, SUITE):
        ...
        run.add_command(
            ...
            base_cmd + ["-S", "--disable-preferred-actions"],
            time_limit=None,
        ...
        run.set_property("algorithm", f"gbfs-lazy-hff-no-pref-ff-{NUM_THREADS}")
        ...
        # multiple algorithms.
        run.set_property(
            "id", [f"gbfs-lazy-hff-no-pref-ff-{NUM_THREADS}", task.domain, task.problem]
        )
